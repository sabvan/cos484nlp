...
def main(experiment_dir: str, lang_code: str, corpus_dir: str, model_name: str, num_bytes: int):
    ...
    # Define characters to exclude (punctuation + whitespace)
    exclude_chars = set(string.punctuation + string.whitespace)
        ...
        with open(file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                for i in range(len(tokens) - 1):
                    ...
                    if a not in exclude_chars and b not in exclude_chars:
                        pair = f"{a} {b}"
                        pair_counts[pair] += 1

    # Save as a list containing a single dict
    with open(f"{lang_dir}/all_pair_counts.json", "w", encoding="utf-8") as fo:
        json.dump([pair_counts], fo, ensure_ascii=False)
    #### ADD ALL PAIRS JSON
