...
def main(
    ...
    # Else, sample languages and weights.
        ...
        # intended language distirbution is based on amount of data
        ...
        for lang_code, weight in zip(languages, weights):
            if lang_code == '.DS_Store':
                continue
            language_files = [f for f in os.listdir(corpus_dir / lang_code) if f.endswith('.txt') and 'truncated' not in f and 'Store' not in f]
